...
import { LayoutDashboard, User, Settings, LogOut, HelpCircle, Bot } from 'lucide-react';
...
export const Sidebar = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  return (
    <>
      ...
      <aside className={cn(
        "fixed left-0 top-0 z-40 h-screen w-64 border-r border-white/10 bg-[#050505]/80 backdrop-blur-2xl transition-transform duration-300 shadow-[4px_0_18px_rgba(0,0,0,0.6)]",
        ...
        <div className="flex h-full flex-col px-4 py-8">
          ...
          <div className="mb-10 flex items-center justify-between">
            <Link 
              href="/dashboard" 
              className="flex items-center gap-3 group/logo cursor-pointer outline-none focus-visible:ring-2 focus-visible:ring-[#00FFD1]/50 rounded-xl py-2 transition-all duration-200 hover:scale-[1.02]"
              onClick={() => onClose()} // Close on mobile when clicking logo
            >
